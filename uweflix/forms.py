...
class access_to_club_form(forms.Form):
    ...
    def clean(self):
        ...
        def __init__(self, *args, **kwargs):
            super(access_to_club_form, self).__init__(*args, **kwargs)
...
class payment_Ticket_form(forms.Form):
    adult_tickets = forms.IntegerField(validators=[MaxValueValidator(100), MinValueValidator(0)], required=False, initial=0, widget=forms.NumberInput(attrs={'class': 'form-control'}))
    student_tickets = forms.IntegerField(validators=[MaxValueValidator(100), MinValueValidator(0)], required=False, initial=0, widget=forms.NumberInput(attrs={'class': 'form-control'}))
    child_tickets = forms.IntegerField(validators=[MaxValueValidator(100), MinValueValidator(0)], required=False, initial=0, widget=forms.NumberInput(attrs={'class': 'form-control'}))
    total_cost = forms.FloatField(label="Total Cost: ", disabled=True, required=False, widget=forms.NumberInput(attrs={'class': 'form-control', 'readonly': 'readonly'}))
...
class Screen_add_form(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        super(Screen_add_form, self).__init__(*args, **kwargs)
        for field in self.fields:
            self.fields[field].widget.attrs.update({'class': 'form-control'})
...
class Film_editing_form(forms.ModelForm):
    ...
    title = forms.CharField(max_length=250, widget=forms.TextInput(attrs={'class': 'form-control rounded'}))
    genre = forms.CharField(max_length=220, widget=forms.TextInput(attrs={'class': 'form-control rounded'}))
    year = forms.IntegerField(widget=forms.NumberInput(attrs={'class': 'form-control rounded'}))
    description = forms.CharField(widget=forms.Textarea(attrs={'class': 'form-control rounded'}))
    poster = forms.ImageField(widget=forms.FileInput(attrs={'class': 'form-control-file rounded'}))
